...
def forecast_rides_sarima(request: ForecastRequest):
    ...
    if request.interval == "week":
        ...
        # df_fun["year_week"] = pd.to_datetime(df_fun["year_week"].astype(str) + '-1', format='%Y-%W-%w')

        df_fun[['year', 'week']] = df_fun['year_week'].astype(str).str.split('-', expand=True)

        # Ensure week is two digits (zero-padded)
        df_fun['week'] = df_fun['week'].str.zfill(2)  # "3" → "03"

        # Reconstruct year-week string in correct format
        df_fun['year_week'] = df_fun['year'] + '-' + df_fun['week']

        # Convert to datetime (Monday of the given week)
        df_fun["year_week"] = pd.to_datetime(df_fun["year_week"] + '-1', format='%Y-%W-%w') #.strftime('%Y-%m-%d')
        # df_fun["year_week"] = df_fun["year_week"] + pd.DateOffset(days=6)

        print(df_fun["year_week"])
        period = 52
    elif request.interval == "month":
        df_fun = df_month
        period = 12
    path = df_fun.groupby(f'year_{request.interval}')["rides"].sum()
    path_nonindexed = path.reset_index()
    # path_nonindexed = df_week[['year_week','rides']]
    model = SARIMAX(path, order=(1, 1, 1), seasonal_order=(1, 1, 1, period))
...
def forecast_rides_gam(request: ForecastRequest):
    if request.interval == "week":
        ...
        df_full = df_month
        path_nonindexed = df_week[['year_month', 'rides']]
